...
int main (int argc, char *argv[])
{
  ...
  if(tcpTypeId.compare("TcpDstcp") == 0 || tcpTypeId.compare("TcpDcVegas") == 0)
    {
      //TcpDstcp uses Instantaneous RTT samples
  ...
  tchRed10.SetRootQueueDisc ("ns3::RedQueueDisc",
                             ...
                             "MinTh", DoubleValue (K10),
                             "MaxTh", DoubleValue (K10));
  ...
  for (std::size_t i = 0; i < 20; i++)
    {
      ...
      clientHelper1.SetAttribute("SendSize", UintegerValue(1448));
  ...
  rxS1R1Throughput.open (tcpTypeId + "-example-s1-r1-throughput.dat", std::ios::out);
  rxS1R1Throughput << "#Time(s) flow thruput(Mb/s)" << std::endl;
  rxS2R2Throughput.open (tcpTypeId + "-example-s2-r2-throughput.dat", std::ios::out);
  rxS2R2Throughput << "#Time(s) flow thruput(Mb/s)" << std::endl;
  rxS3R1Throughput.open (tcpTypeId + "-example-s3-r1-throughput.dat", std::ios::out);
  rxS3R1Throughput << "#Time(s) flow thruput(Mb/s)" << std::endl;
  fairnessIndex.open (tcpTypeId + "-example-fairness.dat", std::ios::out);
  t1QueueLength.open (tcpTypeId + "-example-t1-length.dat", std::ios::out);
  t1QueueLength << "#Time(s) qlen(pkts) qlen(us)" << std::endl;
  t2QueueLength.open (tcpTypeId + "-example-t2-length.dat", std::ios::out);
