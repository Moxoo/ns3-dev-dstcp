 ...
 * \ingroup tcp
 *
 * \brief An implementation of DC-VEGAS.
 */
...
public:
   ...
   * \param [in] beta New beta (congestion estimate) value
   */
  typedef void (* CongestionEstimateTracedCallback)(uint32_t bytesAcked, uint32_t bytesMarked, double beta);
...
private:
  ...
  /**
   * \brief Initialize the value of m_beta
   *
   * \param beat DcVegas beta parameter
   */
  void InitializeDcVegasBeta (double beta);
  void InitializeDcVegasTdcv (uint32_t tdcv);
  ...
  bool m_ceState;                       //!< DcVegas Congestion Experienced state
  ...
  Time m_minRtt;                        //!< Minimum of all DcVegas RTT measurements seen during connection
  /**
