...
#define ATTRIB(name) << " " # name "=\"" << flowI->second.name << "\""
      ...
      << " " "timeFirstTxPacket" "=\"" << flowI->second.timeFirstTxPacket.GetNanoSeconds() << "\""
      // ATTRIB (timeFirstRxPacket)
      << " " "timeFirstRxPacket" "=\"" << flowI->second.timeFirstRxPacket.GetNanoSeconds() << "\""
      // ATTRIB (timeLastTxPacket)
      << " " "timeLastTxPacket" "=\"" << flowI->second.timeLastTxPacket.GetNanoSeconds() << "\""
      // ATTRIB (timeLastRxPacket)
      << " " "timeLastRxPacket" "=\"" << flowI->second.timeLastRxPacket.GetNanoSeconds() << "\""
      // ATTRIB (delaySum)
      << " " "delaySum" "=\"" << flowI->second.delaySum.GetNanoSeconds() << "\""
      // ATTRIB (jitterSum)
      << " " "jitterSum" "=\"" << flowI->second.jitterSum.GetNanoSeconds() << "\""
      // ATTRIB (lastDelay)
      << " " "lastDelay" "=\"" << flowI->second.lastDelay.GetNanoSeconds() << "\""
      ATTRIB (txBytes)
      ATTRIB (rxBytes)
      ATTRIB (txPackets)
      ATTRIB (rxPackets)
